...
import { initializeIndex, checkHealth } from './config/elasticsearch';
...
app.get('/health', async (req, res) => {
  ...
  res.json({
    ...
    service: 'search-service',
    elasticsearch: elasticsearchHealthy ? 'up' : 'down',
...
async function start() {
  try {
    ...
    const server = app.listen(config.port, () => {
      logger.info({ port: config.port }, 'Search service started');
    });
